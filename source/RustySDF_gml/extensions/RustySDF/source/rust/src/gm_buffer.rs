...
// ─── DESERIALIZER (read from GM buffer) ───────────────────────────────────
...
impl<'a> GMBufferReader<'a> {
    ...
    /// Read a null-terminated string slice (zero-allocation)
    pub fn read_string(&mut self) -> Option<&'a str> {
        ...
        self.cursor += nul_pos + 1; // +1 for null terminator
        Some(s)
    }
...
// ─── SERIALIZER (write for GM buffer) ───────────────────────────────────────
...
impl GMBufferWriter {
    ...
    /// Write an array with a builder closure. The closure must write exactly `count` items.
    ...
    /// Write a struct with a known field count.
    ...
    /// Write a struct key-value pair (key is raw string, value is f64)
    pub fn write_struct_kv_f64(&mut self, key: &str, val: f64) {
        self.write_raw_string(key);
        self.write_f64(val);
    }

    /// Write a struct key-value pair (key is raw string, value is u32)
    pub fn write_struct_kv_u32(&mut self, key: &str, val: u32) {
        self.write_raw_string(key);
        self.write_u32(val);
    }

    /// Write a struct key-value pair (key is raw string, value is bool)
    pub fn write_struct_kv_bool(&mut self, key: &str, val: bool) {
        self.write_raw_string(key);
        self.write_bool(val);
    }

    /// Pack a slice of f64 into BUFFER_TYPED_ARRAY format for GameMaker
    pub fn write_f64_typed_array(&mut self, values: &[f64]) {
        self.data.push(250); // BUFFER_TYPED_ARRAY
        self.data.extend_from_slice(&(values.len() as u16).to_le_bytes());
        self.data.push(9); // buffer_f64
        for val in values {
            self.data.extend_from_slice(&val.to_le_bytes());
        }
    }
